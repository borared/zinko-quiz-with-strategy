...
export default function LibrarySearchBar({ value, onChange, resultCount, totalCount }) {
  ...
  return (
    <div className="zk-panel-glass px-4 py-3 flex items-center gap-3 !shadow-none">
      ...
      {hasQuery && (
        <>
          <span className="hidden sm:inline text-xs font-black uppercase tracking-wider text-zk-text/50 shrink-0">
            {resultCount} of {totalCount}
          </span>
          <button
            type="button"
            onClick={() => onChange('')}
            className="inline-flex h-8 w-8 items-center justify-center rounded-lg border-[2px] border-zk-border bg-zk-panel-bg text-zk-text transition-colors hover:bg-zk-bg/30 shrink-0"
