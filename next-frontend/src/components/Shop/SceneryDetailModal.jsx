    ...
    <AnimatePresence>
      {open && item && details && (
        <motion.div
          ...
          className="fixed bottom-0 left-0 right-0 z-[45] flex items-center justify-center bg-black/50 p-4 sm:p-6 md:left-64"
          ...
          >
            ...
            <div className="flex flex-col gap-3 overflow-hidden p-5 sm:gap-4 sm:p-6">
              <div className="shrink-0">
                <span className="inline-flex items-center gap-1.5 rounded-lg border-[2px] border-zk-black bg-zk-yellow px-3 py-1.5 text-[10px] font-black uppercase tracking-[0.15em] text-zk-black">
                  <Sparkles size={12} strokeWidth={3} />
                  Quiz Scenery
                </span>
                <h3
                  id="scenery-detail-title"
                  className="mt-2 font-['Outfit'] text-2xl font-black tracking-tight text-zk-black sm:text-3xl"
                >
                  {item.name}
                </h3>
              ...
              <div className="rounded-xl border-[3px] border-zk-black bg-white p-3 sm:p-4">
                ...
                <ul className="mt-2 grid grid-cols-1 gap-1.5 sm:grid-cols-2 sm:gap-2">
                  {details.perks.map((perk) => (
                    <li
                      key={perk}
                      className="flex items-start gap-2 text-xs font-bold leading-snug text-zk-black"
                    >
                      <span className="mt-1 h-2 w-2 shrink-0 rounded-full border-[2px] border-zk-black bg-zk-yellow" />
                      {perk}
                    </li>
                  ))}
                </ul>
              ...
              {details.hasAudio && (
                <div className="rounded-xl border-[3px] border-zk-black bg-[#5D3FD3]/10 p-3 sm:p-4">
                  <div className="flex flex-wrap items-center justify-between gap-3">
                    <div className="flex items-start gap-3">
                      <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg border-[2px] border-zk-black bg-zk-yellow">
                      ...
                      <div>
                        ...
                        <p className="font-['Outfit'] text-base font-black text-zk-black sm:text-lg">
                          {details.audioLabel}
                        </p>
