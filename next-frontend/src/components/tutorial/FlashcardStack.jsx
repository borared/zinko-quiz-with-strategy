...
export default function FlashcardStack() {
  ...
  const cards = [
    'https://hyfqsjidyxufsatveaih.supabase.co/storage/v1/object/public/tutorial/flashcard1.png',
    'https://hyfqsjidyxufsatveaih.supabase.co/storage/v1/object/public/tutorial/flashcard2.png',
    'https://hyfqsjidyxufsatveaih.supabase.co/storage/v1/object/public/tutorial/flashcard3.png',
  ];

  const handleNext = (e) => {
    e.stopPropagation();
    if (exitIndex !== null) return; // Prevent double clicks during animation
  ...
  return (
    ...
    >
      {cards.map((src, index) => {
        ...
        // Custom state for card currently exiting (flying out to the left)
        if (isExiting) {
          translateX = -35; // Slide left by 35% (shorter move)
          rotate = -8;      // Subtle rotation for a tighter slide
          scale = 0.95;
          zIndex = 40;      // Keep on very top during exit animation
        }
