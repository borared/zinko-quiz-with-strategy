...
const TeamCard = ({ team, label, badge, icon, bgColor, onJoin }) => {
  return (
    <div className="flex flex-col items-center gap-0 w-full max-w-[280px]">
      ...
      {/* Card Box */}
      <div
        className="relative w-full aspect-square border-[4px] border-zk-black shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] flex items-center justify-center cursor-pointer select-none transition-transform duration-200 hover:scale-[1.02]"
        style={{ backgroundColor: bgColor }}
        onClick={onJoin}
      >
        {/* Badge top-left or top-right */}
        <div className="absolute top-[-2px] left-[-2px] bg-white border-[3px] border-zk-black px-3 py-0.5">
