...
const QuizGrid = ({ quizzes, loading, isDiscoveryMode = false, onClone, onDelete, onPreview }) => {
    ...
    <div className="flex flex-col gap-6">
      <div className="flex justify-between items-center">
        <h2 className="font-black text-2xl text-zk-black uppercase tracking-tight">
          {isDiscoveryMode 
            ? (quizzes.length > 0 ? 'Discover Public Quizzes' : 'No Public Quizzes Found')
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {quizzes.map((quiz) => (
          <QuizCard key={quiz.id} quiz={quiz} isDiscoveryMode={isDiscoveryMode} onClone={onClone} onDelete={onDelete} onPreview={onPreview} />
        ))}
